...
<html lang="pl">
    <?php
        ...
        $dbh = 'SELECT * FROM `hotele` WHERE `hotel_id` = ' . $_GET['id'];
        $stmt = $sql->query($dbh);
        $row = $stmt->fetchAll();
        $row = $row[0];
    ?>
    <head>
        ...
        <title><?php echo $row['nazwa']; ?> | MamPieniadze.pl</title>
    </head>
    <body>
        ...
        <main>
            ...
            <h1 class="kierunek-title page-title"><?php echo $row['nazwa']; ?></h1>
            <hr>
            <div class="kierunek-content page-content">
                <div class="container">
                    <div class="row">                     
                        <div class="kierunek-img">
                            <img src="<?php echo $row['img_src']; ?>" alt="<?php echo $row['nazwa']; ?>">
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-md-12">
                            <div class="kierunek-info">
                                <h2>Lokalizacja: </h2>
                                <p><?php echo $row['lokalizacja']; ?></p>
                                <h2>Gwiazdy: </h2>
                                <p><?php echo $row['gwiazdy']; ?></p>
                                <h2>Wyżywienie: </h2>
                                <p><?php switch ($row['wyzywienie']) {
                                    case 'all':
                                        echo "All inclusive";
                                        break;
                                    case 'hb':
                                        echo "Śniadania i obiadokolacje (HB)";
                                        break;
                                    case 'bb':
                                        echo "Śniadania (BB)";
                                        break;
                                ...
                                <h2>Cena: </h2>
                                <h3><?php echo $row['cena']; ?> PLN</h3>
                                <span> / <?php echo rand(5, 12); ?> dni</span>
                                <hr>
                                <?php if(!isset($_COOKIE['user'])): ?>
                                <button type="button" class="btn btn-primary btn-lg" onclick="window.location.href = 'logowanie.php'">Zerezerwuj teraz!</button>
                                <?php else: ?>
                                <button type="button" class="btn btn-primary btn-lg" onclick="window.location.href = 'validphps/zarezerwuj.php?id=<?php echo $row['hotel_id']; ?>'">Zerezerwuj teraz!</button>  
                                <?php endif; ?>
                            </div>
                        </div>
                    </div>
